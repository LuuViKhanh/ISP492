...
from app.modules.missions.schemas import MissionKPIResponse, MissionStatusResponse, ActiveFlightsResponse
...
@router.get("/")
async def get_missions(user: CurrentUser = Depends(get_current_user)):
    return {"message": f"Missions list for user {user.id}"}
